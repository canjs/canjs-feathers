...
(function (root, factory) {
  ...
  // A list of all of the clients for every Model.
  var clients = [];

  var connection = new Map({
    // When the socket changes, all connected Models need
    // to be updated to use the new socket. This deferred
    // will be resolved once they have all updated.
  ...
  var SocketModel = Model.extend({
    ...
    setup: function () {
      ...
      clients.push(this.client = feathersClient(this.resource, socket));
      ...
      this.send = function () {
        ...
        connection.attr('ready').resolve();

        connection.attr('ready').then(function(){
          // Send the request.
